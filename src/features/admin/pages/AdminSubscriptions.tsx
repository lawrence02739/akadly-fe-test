...
const blankDraft: SubscriptionPayload = { name: '', code: '', description: '', price: 0, currency: 'INR', interval: 'monthly', status: 'active', allowedModules: [], userLimit: null, studentLimit: null, contentLimit: null, courseLimit: null };
export function SubscriptionForm({ edit = false }: { edit?: boolean }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    adminApi.subscription(id).then((item) => { if (active) { setDraft({ name: item.name, code: item.code, description: item.description, price: item.price, currency: item.currency, interval: item.interval, status: item.status, allowedModules: item.allowedModules, userLimit: item.userLimit, studentLimit: item.studentLimit, contentLimit: item.contentLimit, courseLimit: item.courseLimit }); setAllowedModulesText(item.allowedModules.join('\n')); setLoadedItem(true); setError(''); } })
