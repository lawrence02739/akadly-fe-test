...
import { adminApi, adminError, type AdminSubscription, type Pagination, type SubscriptionPayload, type SubscriptionQuery } from '../api/admin.api';
...
export function SubscriptionList() {
  ...
  const deleteItem = async (item: AdminSubscription) => {
    ...
    try { await adminApi.deleteSubscription(item.id); toast.success('Subscription deleted.'); if (items.length === 1 && page > 1) setPage(page - 1); else setRevision((value) => value + 1); }
...
export function SubscriptionDetail() {
  ...
  const deleteItem = async () => {
    if (!id || !item || !window.confirm(`Delete ${item.name}? This cannot be undone.`)) return;
    setDeleting(true); setError('');
    try { await adminApi.deleteSubscription(id); toast.success('Subscription deleted.'); navigate('/admin/subscriptions'); }
    catch (cause) { setError(adminError(cause)); }
    finally { setDeleting(false); }
  };
  return <><Link to="/admin/subscriptions" className="inline-flex items-center gap-2 text-sm font-semibold text-[#0C5A69]"><ArrowLeft size={17} /> All subscriptions</Link>{!can('subscription:read') ? <ErrorNotice message="You do not have permission to view subscriptions." /> : loading ? <p className="text-slate-500">Loading subscription...</p> : item ? <><div className="flex flex-wrap items-start justify-between gap-4"><Heading title={item.name} description={item.description || item.code} /><div className="flex gap-2">{can('subscription:update') && <Link to="edit" className={secondary}><Pencil size={16} /> Edit</Link>}{can('subscription:delete') && <button type="button" disabled={deleting} className="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-white px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-50 disabled:opacity-50" onClick={() => void deleteItem()}><Trash2 size={16} /> Delete</button>}</div></div><div className="grid gap-4 sm:grid-cols-2"><div className="rounded-xl border border-slate-200 bg-white p-6"><CreditCard className="text-[#0C5A69]" /><p className="mt-5 text-sm text-slate-500">Price</p><p className="text-2xl font-bold">{money(item)} <span className="text-base font-normal text-slate-500">/{item.interval === 'monthly' ? 'month' : 'year'}</span></p><p className="mt-4 text-sm">Status: <span className="font-semibold capitalize">{item.status}</span></p></div><div className="rounded-xl border border-slate-200 bg-white p-6"><h2 className="font-semibold">Plan details</h2><p className="mt-4 text-sm text-slate-600">Code: {item.code}</p><p className="mt-2 text-sm text-slate-600">Created: {new Date(item.createdAt).toLocaleDateString()}</p></div></div><div className="rounded-xl border border-slate-200 bg-white p-6"><h2 className="font-semibold">Included features</h2>{item.features.length ? <ul className="mt-3 list-inside list-disc space-y-2 text-sm text-slate-600">{item.features.map((feature, index) => <li key={index}>{feature}</li>)}</ul> : <p className="mt-3 text-sm text-slate-500">No features listed.</p>}</div></> : null}{error && <ErrorNotice message={error} retry={() => { setLoading(true); setRevision((value) => value + 1); }} />}</>;
}

const blankDraft: SubscriptionPayload = { name: '', code: '', description: '', price: 0, currency: 'INR', interval: 'monthly', status: 'active', features: [] };
export function SubscriptionForm({ edit = false }: { edit?: boolean }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    adminApi.subscription(id).then((item) => { if (active) { setDraft({ name: item.name, code: item.code, description: item.description, price: item.price, currency: item.currency, interval: item.interval, status: item.status, features: item.features }); setFeaturesText(item.features.join('\n')); setLoadedItem(true); setError(''); } })
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (features.length > 50 || features.some((feature) => feature.length > 200)) { setError('Use no more than 50 features, each up to 200 characters.'); return; }
    if (draft.name.trim().length < 2) { setError('Plan name must contain at least 2 characters.'); return; }
    setSaving(true);
    try {
      const payload = { ...draft, name: draft.name.trim(), code: draft.code.trim().toLowerCase(), description: draft.description.trim(), features };
