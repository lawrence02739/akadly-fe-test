...
function formatStorageLimit(value: number | null) {
  ...
  if (value >= BYTES_PER_GB && value % BYTES_PER_GB === 0) return `${value / BYTES_PER_GB} GB`;
  if (value >= BYTES_PER_MB && value % BYTES_PER_MB === 0) return `${value / BYTES_PER_MB} MB`;
  return `${new Intl.NumberFormat('en-IN').format(value)} bytes`;
}
...
export function SubscriptionForm({ edit = false }: { edit?: boolean }) {
  ...
  const setContentLimitMegabytes = (value: string) => setDraft({
    ...draft,
    contentLimit: value === '' ? null : Math.round(Number(value) * BYTES_PER_MB),
  });
  const contentLimitMegabytes = draft.contentLimit === null ? '' : draft.contentLimit / BYTES_PER_MB;
