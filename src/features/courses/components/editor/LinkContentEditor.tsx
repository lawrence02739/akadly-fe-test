...
export default function LinkContentEditor({
  item,
  isSaving,
  onSaveContent,
  onCancel,
  ...
  const [embedUrl, setEmbedUrl] = useState(item.content?.url ?? '');
  ...
  return (
    <div className="space-y-6">
      {/* Embed URL */}
      <div>
        <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">Embed URL</label>
        <input
          type="url"
          value={embedUrl}
          onChange={e => setEmbedUrl(e.target.value)}
          placeholder="Enter URL to embed(e.g. Google Docs, Figma)"
          className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
        />
      </div>
