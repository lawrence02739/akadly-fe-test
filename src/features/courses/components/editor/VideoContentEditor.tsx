...
import { Upload, Video, Link as LinkIcon, Library, Image as ImageIcon } from 'lucide-react';
import { useUpload } from '../../hooks/useUpload';

export default function VideoContentEditor({ item }: { item: any }) {
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {uploadType === 'upload' && (
          <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 flex flex-col items-center justify-center text-center hover:bg-slate-50 transition-colors">
            ...
            <p className="text-xs text-slate-500 mb-4">MP4, WebM or OGG (max. 2GB)</p>
            <input type="file" accept="video/*" onChange={handleFileChange} className="hidden" id="video-upload" />
            <label htmlFor="video-upload" className="px-4 py-2 bg-white border border-slate-300 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50 cursor-pointer">
              Select Video
            </label>
            ...
            {file && !isPending && progress === 0 && (
              <button onClick={handleUpload} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700">
                Start Upload
              </button>
            )}
          </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            <div className="w-32 h-20 bg-slate-100 border border-slate-200 rounded-lg flex items-center justify-center text-slate-400">
